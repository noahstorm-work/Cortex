...
async function getDashboardData(userId: string) {
  ...
  const [docResult, chunkResult, searchResult, recentDocs, recentSearches] = await Promise.all([
    ...
    supabase
      ...
      .in(
        "document_id",
        (await supabase.from("documents").select("id").eq("user_id", userId)).data?.map(
          (d) => d.id
        ) || []
      ),
    supabase
  ...
  return {
    ...
    recentDocuments: (recentDocs.data || []) as Pick<
      Document,
      "id" | "title" | "status" | "created_at"
    >[],
    recentSearches: (recentSearches.data || []) as Pick<
      SearchHistoryItem,
      "id" | "query" | "created_at"
    >[],
  };
        ...
        <ErrorBoundary>
          <div className="rounded-2xl border border-border/50 bg-card/50 p-6 shadow-sm animate-fade-in-up stagger-5">
            <div className="mb-5 flex items-center justify-between">
              ...
              <Link
                href="/documents"
                className="text-xs text-muted-foreground hover:text-foreground transition-colors inline-flex items-center gap-1 focus-visible:ring-2 focus-visible:ring-teal-400/40 rounded-lg group"
              >
                View all{" "}
            ...
            {data.recentDocuments.length === 0 ? (
              ...
              <div className="space-y-2">
                {data.recentDocuments.map((doc) => (
                  <div
                    key={doc.id}
                    className="group flex items-center justify-between rounded-xl bg-muted/30 px-4 py-2.5 transition-all duration-300 hover:bg-muted/50"
                  >
                    <span className="text-sm font-medium text-foreground truncate">
                      {doc.title}
                    </span>
                    <span
                      className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${getStatusBadgeClass(doc.status)}`}
                    >
                      {doc.status}
                    </span>
                  </div>
            ...
            {data.recentSearches.length === 0 ? (
              ...
              <div className="space-y-2">
                {data.recentSearches.map((s) => (
                  <div
                    key={s.id}
                    className="group flex items-center gap-3 rounded-xl bg-muted/30 px-4 py-2.5 transition-all duration-300 hover:bg-muted/50"
                  >
