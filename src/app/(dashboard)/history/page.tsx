...
export default function HistoryPage() {
  ...
  const toggleSavedMutation = useMutation({
    mutationFn: async ({ id, currentSaved }: { id: string; currentSaved: boolean }) => {
      const newSaved = !currentSaved;
      const supabase = createClient();
      await supabase.from("search_history").update({ saved: newSaved }).eq("id", id);
      return { id, newSaved };
    },
    onMutate: async ({ id, currentSaved }) => {
      const newSaved = !currentSaved;
      queryClient.setQueryData<SearchHistoryItem[]>(["search-history", userId], (old) =>
        old ? old.map((h) => (h.id === id ? { ...h, saved: newSaved } : h)) : old
      );
    },
    onError: () => {
  ...
  const bulkDeleteMutation = useMutation({
    ...
    onMutate: async (ids) => {
      queryClient.setQueryData<SearchHistoryItem[]>(["search-history", userId], (old) =>
        old ? old.filter((h) => !ids.includes(h.id)) : old
      );
  ...
  if (error) {
    return (
      <div
        role="alert"
        aria-live="polite"
        className="rounded-2xl border border-destructive/20 bg-destructive/5 p-6 text-sm text-destructive"
      >
        <div className="flex items-center gap-2">
          <AlertTriangle className="h-4 w-4 shrink-0" aria-hidden="true" />
          <span>Failed to load search history</span>
        </div>
        <button
          onClick={() => loadHistory()}
          className="mt-3 inline-flex items-center gap-2 text-xs font-medium text-accent hover:text-accent/80 focus-visible:ring-2 focus-visible:ring-teal-400/40 rounded-lg transition-colors"
        >
          Try again
      ...
      <div className="space-y-8 animate-fade-in-up">
        ...
        >
          <div className="relative flex-1 min-w-[160px] max-w-xs">
            <Search
              className="absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground/40"
              aria-hidden="true"
            />
            <Input
              ...
              >
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  ...
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-teal-400/10 to-teal-600/10 group-hover:from-teal-400/20 group-hover:to-teal-600/20 transition-all duration-300">
                    <History
                      className="h-4 w-4 text-teal-400/70 group-hover:text-teal-400 transition-colors duration-300"
                      aria-hidden="true"
                    />
                  </div>
                ...
                <div className="flex items-center gap-1 shrink-0 ml-4">
                  ...
                  <button
                    onClick={() =>
                      toggleSavedMutation.mutate({ id: item.id, currentSaved: item.saved ?? false })
                    }
