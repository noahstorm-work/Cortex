...
describe("SearchHistory", () => {
  ...
  it("shows error state on fetch failure", async () => {
    ...
    render(<SearchHistory />);
    await waitFor(() =>
      expect(screen.getByText("Could not load search history")).toBeInTheDocument()
    );
  ...
  it("shows empty state when no history", async () => {
    ...
    render(<SearchHistory />);
    await waitFor(() =>
      expect(screen.getByText("No searches yet—try searching above.")).toBeInTheDocument()
    );
  });
  ...
  it("shows no matching searches message when filter yields no results", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText("No matching searches for this filter.")).toBeInTheDocument()
    );
  });
