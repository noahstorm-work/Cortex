    ...
    <div className="flex gap-1">
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem onClick={() => download(jsonData, `cortex-search-${Date.now()}.json`, "application/json")}>
            Export as JSON
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => download(toCSV(results), `cortex-search-${Date.now()}.csv`, "text/csv")}>
            Export as CSV
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => download(toMarkdown(results), `cortex-search-${Date.now()}.md`, "text/markdown")}>
            Export as Markdown
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
